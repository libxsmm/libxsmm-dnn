...
LIBXSMM_INLINE void copy_buf_uint16(unsigned short* src, unsigned short* dst, size_t size) {
  int i;
#if defined(_OPENMP)
  LIBXSMM_OMP_VAR(i);
# pragma omp parallel for private(i)
...
LIBXSMM_INLINE void matrix_copy_NC_to_NCNC_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int T, int N, int C, int bn, int bc)
{
  int t, n1, n2, c1, c2;
  int nBlocks = N/bn;
  int cBlocks = C/bc;
  LIBXSMM_VLA_DECL(3, libxsmm_bfloat8, real_src, src, N, C);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, real_dst, dst, nBlocks, cBlocks, bn, bc);
...
LIBXSMM_INLINE void matrix_copy_NC_to_NCNC_bf8_serial(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int T, int N, int C, int bn, int bc)
{
  int t, n1, n2, c1, c2;
  int nBlocks = N/bn;
  int cBlocks = C/bc;
...
LIBXSMM_INLINE void matrix_copy_NCNC_vnniT_to_NC_bf16(libxsmm_bfloat16 *src, libxsmm_bfloat16 *dst, int T, int N, int C, int bn, int bc)
{
  int t, n1, n2, c1, c2;
  int nBlocks = N/bn;
  int cBlocks = C/bc;
  int vnni_block = libxsmm_cpuid_dot_pack_factor(LIBXSMM_DATATYPE_BF16);
  LIBXSMM_VLA_DECL(3, libxsmm_bfloat16, real_dst, dst, N, C);
  LIBXSMM_VLA_DECL(6, libxsmm_bfloat16, real_src, src, nBlocks, cBlocks, bc/vnni_block, bn, vnni_block);
    ...
    for (n1 = 0; n1 < nBlocks; n1++) {
      for (c1 = 0; c1 < cBlocks; c1++) {
        for (n2 = 0; n2 < bn; n2++) {
          for (c2 = 0; c2 < bc; c2++) {
            LIBXSMM_VLA_ACCESS(3, real_dst, t, n1*bn+n2, c1*bc+c2, N, C) =
              LIBXSMM_VLA_ACCESS(6, real_src, t, n1, c1, c2/vnni_block, n2, c2%vnni_block, nBlocks, cBlocks, bc/vnni_block, bn, vnni_block);
          }
        }
      }
    }
...
LIBXSMM_INLINE void matrix_copy_NCNC_to_NC_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int T, int N, int C, int bn, int bc)
{
  int t, n1, n2, c1, c2;
  int nBlocks = N/bn;
  int cBlocks = C/bc;
  LIBXSMM_VLA_DECL(3, libxsmm_bfloat8, real_dst, dst, N, C);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, real_src, src, nBlocks, cBlocks, bn, bc);
...
LIBXSMM_INLINE void matrix_copy_KC_to_CK(float *src, float *dst, int C, int K)
{
  int k1, c1;
  LIBXSMM_VLA_DECL(2, float, real_src, src, C);
  LIBXSMM_VLA_DECL(2, float, real_dst, dst, K);
...
# pragma omp parallel for private(k1,c1)
  ...
  for (k1 = 0; k1 < K; k1++) {
    for (c1 = 0; c1 < C; c1++) {
      LIBXSMM_VLA_ACCESS(2, real_dst, c1, k1, K) =
        LIBXSMM_VLA_ACCESS(2, real_src, k1, c1, C);
    }
  }
}

LIBXSMM_INLINE void matrix_copy_KC_to_CK_bf16(libxsmm_bfloat16 *src, libxsmm_bfloat16 *dst, int C, int K)
{
  int k1, c1;
  LIBXSMM_VLA_DECL(2, libxsmm_bfloat16, real_src, src, C);
...
LIBXSMM_INLINE void matrix_copy_CK_to_KCCK_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int C, int K, int bc, int bk)
{
  ...
  int kBlocks = K/bk;
  int cBlocks = C/bc;
  int vnni_block = libxsmm_cpuid_dot_pack_factor(LIBXSMM_DATATYPE_BF8);
  LIBXSMM_VLA_DECL(2, libxsmm_bfloat8, real_src, src, K);
...
LIBXSMM_INLINE void matrix_copy_CK_to_CKKC_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int C, int K, int bc, int bk)
{
  int k1, k2, c1, c2;
  int kBlocks = K/bk;
  int cBlocks = C/bc;
  int vnni_block = libxsmm_cpuid_dot_pack_factor(LIBXSMM_DATATYPE_BF8);
  LIBXSMM_VLA_DECL(2, libxsmm_bfloat8, real_src, src, K);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, real_dst, dst, kBlocks, bk/vnni_block, bc, vnni_block);
...
LIBXSMM_INLINE void matrix_copy_KC_to_KCCK_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int C, int K, int bc, int bk)
{
  int k1, k2, c1, c2;
  int kBlocks = K/bk;
  int cBlocks = C/bc;
...
LIBXSMM_INLINE void matrix_copy_KCCK_to_KC_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int C, int K, int bc, int bk)
{
  int k1, k2, c1, c2;
  int kBlocks = K/bk;
  int cBlocks = C/bc;
  int vnni_block = libxsmm_cpuid_dot_pack_factor(LIBXSMM_DATATYPE_BF8);
  LIBXSMM_VLA_DECL(2, libxsmm_bfloat8, real_dst, dst, C);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, real_src, src, cBlocks, bc/vnni_block, bk, vnni_block);
...
LIBXSMM_INLINE void matrix_copy_KCCK_to_CK_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int C, int K, int bc, int bk)
{
  int k1, k2, c1, c2;
  int kBlocks = K/bk;
  int cBlocks = C/bc;
  int vnni_block = libxsmm_cpuid_dot_pack_factor(LIBXSMM_DATATYPE_BF8);
  LIBXSMM_VLA_DECL(2, libxsmm_bfloat8, real_dst, dst, K);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, real_src, src, cBlocks, bc/vnni_block, bk, vnni_block);
...
LIBXSMM_INLINE void matrix_copy_KCCK_to_CKKC_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int C, int K, int bc, int bk)
{
  int k1, k2, c1, c2;
  int kBlocks = K/bk;
  int cBlocks = C/bc;
...
LIBXSMM_INLINE void tensor_copy_NCHW_to_NCHWc_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int N, int C, int H, int W, int bc)
{
  int n, h, w, c1, c2;
  int cBlocks = C/bc;
  LIBXSMM_VLA_DECL(4, libxsmm_bfloat8,  in, src, C, H, W);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, out, dst, cBlocks, H, W, bc);
...
LIBXSMM_INLINE void tensor_pollute_rim_NCHWc_bf8(libxsmm_bfloat8 *dst, int N, int C, int H, int W, int bc, int pad_h, int pad_w, float polute_val)
{
  int n, h, w, c1, c2;
  int cBlocks = C/bc;
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, out, dst, cBlocks, H, W, bc);
...
LIBXSMM_INLINE void tensor_copy_NCHWc_to_NCHW_bf8(libxsmm_bfloat8 *src, libxsmm_bfloat8 *dst, int N, int C, int H, int W, int bc)
{
  int n, h, w, c1, c2;
  int cBlocks = C/bc;
  LIBXSMM_VLA_DECL(4, libxsmm_bfloat8, out, dst, C, H, W);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8,  in, src, cBlocks, H, W, bc);
...
LIBXSMM_INLINE void tensor_cvt_copy_KCRS_to_KCRSck_bf8(float *src, libxsmm_bfloat8 *dst, int K, int C, int R, int S, int bc, int bk)
{
  ...
  int kBlocks = K/bk;
  LIBXSMM_VLA_DECL(4, float, in, src, C, R, S);
  LIBXSMM_VLA_DECL(7, libxsmm_bfloat8, out, dst, cBlocks, R, S, bc/4, bk, 4);
    ...
    for (k2 = 0; k2 < bk; k2++) {
      for (c1 = 0; c1 < cBlocks; c1++) {
        for (c2 = 0; c2 < bc; c2++) {
          for (r = 0; r < R; r++) {
            for (s = 0; s < S; s++) {
              libxsmm_rne_convert_fp32_bf8( &LIBXSMM_VLA_ACCESS(4, in,  k1*bk+k2, c1*bc+c2, r, s, C, R, S),
                                             &LIBXSMM_VLA_ACCESS(7, out, k1,     c1,         r, s, c2/4, k2, c2%4, cBlocks, R, S, bc/4, bk, 4),     1);
            }
          }
        }
      }
          ...
          for ( wi = 0, wo = 0; wi < ifw; wi += sw, wo++ ) {
            ...
            *output_ptr2 = o;
            ...
            if ( (param->fuse_type == 4) || (param->fuse_type == 5) ) {
              ...
              *relumask_ptr2 = (unsigned char)(( o <= 0.0 ) ? 0x0 : 1/*(1 << (i%8))*/ );
            }
          }
        }
      }
      ...
      for ( fmg = 0; fmg < nFMG; fmg++ ) {
        for ( hi = 0, ho = 0; hi < ifh; hi += sh, ho++ ) {
          for ( wi = 0, wo = 0; wi < ifw; wi += sw, wo++) {
            ...
            *del_input_ptr = t0_val * ((gamma_ptr[g*nFMG+fmg] * ghw * del_output_val) - d2_val - ((input_val - expectval_ptr[img*nG+g]) * d1_val * rcpstddev_ptr[img*nG+g] * rcpstddev_ptr[img*nG+g]/*(1.0/(variance_ptr[img*nG+g]+eps))*/));
          }
        }
      }
    }
                   ...
                   float *icfogoldt, float *wgold, float *rgold, float *scratch )
{
  ...
  for (j = 0; j < t; ++j) {
...
#if defined(TWO_GEMMS)
    /* icfo += W * x */
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K4, &N, &C, &alpha, wgold, &K4, &LIBXSMM_VLA_ACCESS(2, xgold, j, 0, N * C), &C, &beta, &LIBXSMM_VLA_ACCESS(3, icfogold, j, 0, 0, N, 4 * K), &K4);
    /* icfo += R * h */
    if (j == 0) {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K4, &N, &K, &alpha, rgold, &K4, hpgold, &K, &beta, &LIBXSMM_VLA_ACCESS(3, icfogold, 0, 0, 0, N, 4 * K), &K4);
    } else {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K4, &N, &K, &alpha, rgold, &K4, &LIBXSMM_VLA_ACCESS(2, hgold, j-1, 0, K * N), &K, &beta, &LIBXSMM_VLA_ACCESS(3, icfogold, j, 0, 0, N, 4 * K), &K4);
    }
#else
    /* Concatenate x and h */
    ...
    /* icfo += (W * x) + (R * h) */
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K4, &N, &CK, &alpha, wgold, &K4, xhgold, &CK, &beta, &LIBXSMM_VLA_ACCESS(3, icfogold, j, 0, 0, N, 4 * K), &K4);
#endif
#if defined(PROFILE)
                       ...
                       float *dxgoldt, float *dcspgold, float *dhpgold, float *scratch )
{
  ...
  for (j = t-1; j >= 0; --j) {
    ...
    if (j > 0) {
      /* compute dout */
      LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &K, &N, &K4, &alpha, rgold, &K4, &LIBXSMM_VLA_ACCESS(3, dicfogold, j, 0, 0, N, 4 * K), &K4, &beta0, &LIBXSMM_VLA_ACCESS(2, doutgold, j-1, 0, K * N), &K);
    } else {
      /* compute dhp */
      LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &K, &N, &K4, &alpha, rgold, &K4, &LIBXSMM_VLA_ACCESS(3, dicfogold, 0, 0, 0, N, 4 * K), &K4, &beta0, dhpgold, &K);
    }

    /* compute dx */
    LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &C, &N, &K4, &alpha, wgold, &K4, &LIBXSMM_VLA_ACCESS(3, dicfogold, j, 0, 0, N, 4 * K), &K4, &beta, &LIBXSMM_VLA_ACCESS(2, dxgold, j, 0, N * C), &C);

    /* compute dw */
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K4, &C, &N, &alpha, &LIBXSMM_VLA_ACCESS(3, dicfogold, j, 0, 0, N, 4 * K), &K4, &LIBXSMM_VLA_ACCESS(2, xgold, j, 0, N * C), &C, &beta, dwgold, &K4);

    /* compute dr */
    if (j == 0) {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K4, &K, &N, &alpha, &LIBXSMM_VLA_ACCESS(3, dicfogold, j, 0, 0, N, 4 * K), &K4, hpgold, &K, &beta, drgold, &K4);
    } else {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K4, &K, &N, &alpha, &LIBXSMM_VLA_ACCESS(3, dicfogold, j, 0, 0, N, 4 * K), &K4, &LIBXSMM_VLA_ACCESS(2, hgold, j-1, 0, K * N), &K, &beta, drgold, &K4);
    }
#else
    LIBXSMM_UNUSED(rgold); LIBXSMM_UNUSED(drgold);
    LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &CK, &N, &K4, &alpha, wgold, &K4, &LIBXSMM_VLA_ACCESS(3, dicfogold, j, 0, 0, N, 4 * K), &K4, &beta0, dxhgold, &CK);
    ...
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K4, &CK, &N, &alpha, &LIBXSMM_VLA_ACCESS(3, dicfogold, j, 0, 0, N, 4 * K), &K4, xhgold, &CK, &beta, dwgold, &K4);
#endif
#if defined(PROFILE)
    Gbl_blas_end = libxsmm_timer_tick();
    Gbl_blas_total += libxsmm_timer_duration(Gbl_blas_start, Gbl_blas_end);
                  ...
                  float *it, float *ct, float *ft, float *ot )
{
  ...
  for (j = 0; j < t; ++j) {
    ...
    /* i_t += W_i * x_t */
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K, &N, &C, &alpha, wi, &K, &LIBXSMM_VLA_ACCESS(2, x, j, 0, N * C), &C, &beta, &LIBXSMM_VLA_ACCESS(2, i, j, 0, K * N), &K);
    /* i_t += R_i * h_{t-1} */
    if (0 == j) {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K, &N, &K, &alpha, ri, &K, hp,                                       &K, &beta, &LIBXSMM_VLA_ACCESS(2, i, j, 0, K * N), &K);
    } else {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K, &N, &K, &alpha, ri, &K, &LIBXSMM_VLA_ACCESS(2, h, j-1, 0, K * N), &K, &beta, &LIBXSMM_VLA_ACCESS(2, i, j, 0, K * N), &K);
    }
    /* i_t = sigmoid(i_t) */
    ...
    /* c_t += W_c * x_t */
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K, &N, &C, &alpha, wc, &K, &LIBXSMM_VLA_ACCESS(2, x, j, 0, N * C), &C, &beta, &LIBXSMM_VLA_ACCESS(2, c, j, 0, K * N), &K);
    /* c_t += R_c * h_{t-1} */
    if (0 == j) {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K, &N, &K, &alpha, rc, &K, hp,                                       &K, &beta, &LIBXSMM_VLA_ACCESS(2, c, j, 0, K * N), &K);
    } else {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K, &N, &K, &alpha, rc, &K, &LIBXSMM_VLA_ACCESS(2, h, j-1, 0, K * N), &K, &beta, &LIBXSMM_VLA_ACCESS(2, c, j, 0, K * N), &K);
    }
    /* c_t = sigmoid(c_t) */
    ...
    /* f_t += W_f * x_t */
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K, &N, &C, &alpha, wf, &K, &LIBXSMM_VLA_ACCESS(2, x, j, 0, N * C), &C, &beta, &LIBXSMM_VLA_ACCESS(2, f, j, 0, K * N), &K);
    /* f_t += R_f * o_t */
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transb, &K, &N, &K, &alpha, rf, &K, &LIBXSMM_VLA_ACCESS(2, o, j, 0, K * N), &K, &beta, &LIBXSMM_VLA_ACCESS(2, f, j, 0, K * N), &K);
    /* f_t = tanh(f_t) */
...
#   pragma omp parallel for private(l, p) LIBXSMM_OPENMP_COLLAPSE(2)
    ...
    /* do = {R_f}^T * df */
    LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &K, &N, &K, &alpha, rf, &K, dfD, &K, &beta0, doD, &K);
...
#     pragma omp parallel for private(l, p) LIBXSMM_OPENMP_COLLAPSE(2)
    ...
    /* dx_t  = {W_i}^T * di */
    LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &C, &N, &K, &alpha, wi, &K, diD, &K, &beta0, &LIBXSMM_VLA_ACCESS(3, dx, j, 0, 0, N, C), &C);
    /* dx_t += {W_c}^T * dc */
    LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &C, &N, &K, &alpha, wc, &K, dcD, &K, &beta,  &LIBXSMM_VLA_ACCESS(3, dx, j, 0, 0, N, C), &C);
    /* dx_t += {W_f}^T * df */
    LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &C, &N, &K, &alpha, wf, &K, dfD, &K, &beta,  &LIBXSMM_VLA_ACCESS(3, dx, j, 0, 0, N, C), &C);
    /* dh_{t-1}  = {R_i}^T * di */
    /* dh_{t-1} += {R_c}^T * dc */
    if (0 == j) {
      LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &K, &N, &K, &alpha, ri, &K, diD, &K, &beta0, dhpD, &K);
      LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &K, &N, &K, &alpha, rc, &K, dcD, &K, &beta,  dhpD, &K);
    } else {
      LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &K, &N, &K, &alpha, ri, &K, diD, &K, &beta0, doutD, &K);
      LIBXSMM_GEMM_SYMBOL(float)(&transaT, &transb, &K, &N, &K, &alpha, rc, &K, dcD, &K, &beta,  doutD, &K);
    }
...
#     pragma omp parallel for private(l, p) LIBXSMM_OPENMP_COLLAPSE(2)
    ...
    /* dw_c += dc * {x_t}^T */
    /* dw_f += df * {x_t}^T */
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K, &C, &N, &alpha, diD, &K, &LIBXSMM_VLA_ACCESS(3, x, j, 0, 0, N, C), &C, &beta, dwi, &K);
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K, &C, &N, &alpha, dcD, &K, &LIBXSMM_VLA_ACCESS(3, x, j, 0, 0, N, C), &C, &beta, dwc, &K);
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K, &C, &N, &alpha, dfD, &K, &LIBXSMM_VLA_ACCESS(3, x, j, 0, 0, N, C), &C, &beta, dwf, &K);
    /* dr_i += di * {o_t}^T */
    /* dr_c += dc * {o_t}^T */
    /* dr_f += df * {h_{t-1}}^T */
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K, &K, &N, &alpha, diD, &K, &LIBXSMM_VLA_ACCESS(3, o, j, 0, 0, N, K), &K, &beta, dri, &K);
    LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K, &K, &N, &alpha, dcD, &K, &LIBXSMM_VLA_ACCESS(3, o, j, 0, 0, N, K), &K, &beta, drc, &K);
    if (0 == j) {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K, &K, &N, &alpha, dfD, &K, &LIBXSMM_VLA_ACCESS(2, hp, 0, 0, K),        &K, &beta, drf, &K);
    } else {
      LIBXSMM_GEMM_SYMBOL(float)(&transa, &transbT, &K, &K, &N, &alpha, dfD, &K, &LIBXSMM_VLA_ACCESS(3, h, j-1, 0, 0, N, K), &K, &beta, drf, &K);
    }
    /* compute db */
#if defined(_OPENMP)
#   pragma omp parallel for private(l, p)
