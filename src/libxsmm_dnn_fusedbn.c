...
LIBXSMM_API libxsmm_dnn_bn_fwd_config setup_libxsmm_dnn_bn_fwd(libxsmm_blasint N, libxsmm_blasint C, libxsmm_blasint H, libxsmm_blasint W, libxsmm_blasint bc,
  ...
  res.helper_add_kernel = libxsmm_dispatch_meltw_binary(LIBXSMM_MELTW_TYPE_BINARY_ADD, binary_shape, binary_flags);
  ...
  if (res.use_hw_blocking == 0)
    ...
    unary_shape           = libxsmm_create_meltw_unary_shape(res.bc, res.H*res.W / res.num_HW_blocks, ld, tmp_ld, res.datatype_in, res.datatype_comp, res.datatype_comp);
  unary_flags             = LIBXSMM_MELTW_FLAG_UNARY_REDUCE_COLS;
  res.reduce_beta0_kernel = libxsmm_dispatch_meltw_unary(LIBXSMM_MELTW_TYPE_UNARY_REDUCE_X_X2_OP_ADD, unary_shape, unary_flags);
  ...
  res.reduce_beta1_kernel = libxsmm_dispatch_meltw_unary(LIBXSMM_MELTW_TYPE_UNARY_REDUCE_X_X2_OP_ADD, unary_shape, unary_flags);
...
LIBXSMM_API libxsmm_dnn_bn_bwd_config setup_libxsmm_dnn_bn_bwd(libxsmm_blasint N, libxsmm_blasint C, libxsmm_blasint H, libxsmm_blasint W, libxsmm_blasint bc,
  ...
  unary_flags            = LIBXSMM_MELTW_FLAG_UNARY_NONE;
  res.helper_copy_kernel = libxsmm_dispatch_meltw_unary(LIBXSMM_MELTW_TYPE_UNARY_IDENTITY, unary_shape, unary_flags);
  ...
  op_metadata[0].op_arg_pos   = -1;
  libxsmm_meqn_push_back_binary_op(op_metadata[0], LIBXSMM_MELTW_TYPE_BINARY_ADD, res.datatype_comp, binary_flags); /* dbeta_tmp [HW, bc] */

  unary_flags                 = LIBXSMM_MELTW_FLAG_UNARY_REDUCE_COLS;
  op_metadata[1].eqn_idx      = my_eqn12;
  op_metadata[1].op_arg_pos   = -1;
  libxsmm_meqn_push_back_unary_op(op_metadata[1], LIBXSMM_MELTW_TYPE_UNARY_REDUCE_X_OP_ADD, res.datatype_comp, unary_flags); /* [HW, bc] -> [bc] */
  ...
  /* din long equation */
  my_eqn16 = libxsmm_meqn_create();                          /* din = a * dout + (b * inp + c) */
  ...
  res.din_func = libxsmm_dispatch_meqn( my_eqn16, eqn_out_arg_shape );
...
LIBXSMM_API void destroy_libxsmm_dnn_bn_fwd(libxsmm_dnn_bn_fwd_config* cfg) {
  ...
  /* when/if libxsmm_meqn_destroy gets added, destructords for equations should go here */
}
