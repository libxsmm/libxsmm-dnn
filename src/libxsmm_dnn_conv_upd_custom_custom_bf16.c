...
LIBXSMM_API void libxsmm_dnn_conv_upd_exec_bf16( libxsmm_dnn_conv_config cfg, const libxsmm_bfloat16* in_act_ptr, const libxsmm_bfloat16* dout_act_ptr, libxsmm_bfloat16* dfilter_ptr,
  ...
  LIBXSMM_VLA_DECL(7, libxsmm_bfloat16, weight_dst, (libxsmm_bfloat16*)filter_dst_ptr, cfg.blocksifm, cfg.R, cfg.S, cfg.ifmblock/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
  ...
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat16, tr_output, (libxsmm_bfloat16*) scratch_tr_output, cfg.blocksofm, cfg.output_pixels/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
  LIBXSMM_VLA_DECL(6, libxsmm_bfloat16, tr_output_2, (libxsmm_bfloat16*) scratch_tr_output, cfg.blocksofm, OFHP, cfg.ofwp_extended/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
        ...
        for (img = my_img_start; img < my_img_end; img++) {
          if (cfg.upd_padding_copy == 1) {
            ...
            for (ifm1 = 0; ifm1 < cfg.blocksifm; ifm1++) {
              ...
              if ((cfg.input_pixels - cfg.ifhp * cfg.ifwp) > 0) {
                unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(4, tr_input, img, ifm1, 0, cfg.ifhp * cfg.ifwp, cfg.blocksifm, cfg.ifmblock, cfg.input_pixels);
                cfg.input_zero_remaining_pixels_bf16( &unary_param );
              }
            }
          }
        }
      ...
      if (cfg.upd_padding_copy == 1) {
        for (img = my_img_start; img < my_img_end; img++) {
          for (ofm1 = 0; ofm1 < cfg.blocksofm; ofm1++) {
            zero_ptr_out = (libxsmm_bfloat16*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, 0, 0, 0, cfg.blocksofm, cfg.output_pixels/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
            unary_param.out.primary = (void*) zero_ptr_out;
            cfg.zero_ofmblock_output_pixels_bf16( &unary_param );
            if (cfg.ofwp % cfg.req_dot_modfree_divisor == 0) {
              for (oj = 0; oj < cfg.ofhp; oj++) {
                ...
                unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, (oj*OFWP)/2, 0, 0, cfg.blocksofm, cfg.output_pixels/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
              ...
              for (oj = 0; oj < cfg.ofhp; oj++) {
                for (oi = 0; oi < cfg.ofwp; oi++) {
                  for (ofm2 = 0; ofm2 < cfg.ofmblock; ofm2++) {
                    LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, (oj*OFWP+oi)/cfg.req_dot_modfree_divisor, ofm2, (oj*OFWP+oi)%cfg.req_dot_modfree_divisor, cfg.blocksofm, cfg.output_pixels/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor) =
        ...
        for (img = my_img_start; img < my_img_end; img++) {
          for (ofm1 = 0; ofm1 < cfg.blocksofm; ofm1++) {
            ...
            unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, 0, 0, 0, cfg.blocksofm, cfg.output_pixels/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
            cfg.vnni_output_compute_pixels_bf16( &unary_param );
            if (cfg.upd_remaining_pixels > 0) {
              unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, ((cfg.compute_pixels+cfg.req_dot_modfree_divisor-1)/cfg.req_dot_modfree_divisor), 0, 0, cfg.blocksofm, cfg.output_pixels/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
      ...
      for (img = my_img_start; img < my_img_end; img++) {
        for (ofm1 = 0; ofm1 < cfg.blocksofm; ofm1++) {
          for (oj = 0; oj < cfg.ofh; oj++) {
            unary_param.in.primary = (void*) &LIBXSMM_VLA_ACCESS(5, output, img, ofm1, oj, 0, 0, cfg.blocksofm, cfg.ofhp, cfg.ofwp, cfg.ofmblock);
            unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(6, tr_output_2, img, ofm1, oj, 0, 0, 0, cfg.blocksofm, OFHP, cfg.ofwp_extended/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
            cfg.vnni_output_w_pixels_bf16( &unary_param );
          }
        }
      }
            ...
            for (ifmb = 0; ifmb < cfg.blocksifm; ifmb += cfg.block_upd_ifm) {
              for (ofm1 = ofmb; ofm1 < LIBXSMM_MIN(ofmb+cfg.block_upd_ofm, cfg.blocksofm); ofm1++ ) {
                ...
                for (j=0; j < cfg.batchreduce_h_pixels; j++) {
                  ...
                  unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(6, tr_output_2, img, 0, j, 0, 0, 0, cfg.blocksofm, OFHP, cfg.ofwp_extended/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
                  cfg.vnni_output_w_pixels_bf16( &unary_param );
                }
                for (ifm1 = ifmb; ifm1 < LIBXSMM_MIN(ifmb+cfg.block_upd_ifm, cfg.blocksifm); ifm1++) {
                  ...
                  for (kj = 0; kj < cfg.R; ++kj) {
                    for (ki = 0; ki < cfg.S; ++ki) {
                      ...
                      gemm_param.a.primary    = (void*) &LIBXSMM_VLA_ACCESS(6, tr_output_2, img, 0, 0, 0, 0, 0, cfg.blocksofm, OFHP, cfg.ofwp_extended/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
                      ...
                      if ((oj + cfg.batchreduce_h_pixels >= cfg.ofh) && (img == my_img_end - 1)) {
                        ...
                        unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(7, weight_dst, ofm1, ifm1, kj, ki, 0, 0, 0, cfg.blocksifm, cfg.R, cfg.S, cfg.ifmblock/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
                ...
                for (ifm1 = ifmb; ifm1 < LIBXSMM_MIN(ifmb+cfg.block_upd_ifm, cfg.blocksifm); ifm1++) {
                  for (kj = 0; kj < cfg.R; ++kj) {
                    for (ki = 0; ki < cfg.S; ++ki) {
                      ...
                      gemm_param.op.tertiary  = (void*) &n_blocks;
                      gemm_param.a.primary    = (void*) &LIBXSMM_VLA_ACCESS(6, tr_output_2, img, ofm1, oj, 0, 0, 0, cfg.blocksofm, OFHP, cfg.ofwp_extended/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
                      if (cfg.on_the_fly_input_packing == 1) {
                        gemm_param.b.primary    = (void*) &LIBXSMM_VLA_ACCESS(5, tr_input_2, img, 0, 0, 0, 0, cfg.blocksifm, cfg.ifmblock, IFHP, cfg.ifwp_extended);
                      } else {
                ...
                for (ifm1 = ifmb; ifm1 < LIBXSMM_MIN(ifmb+block_ifm, my_ifm_end); ifm1++) {
                  for (kj = my_R_start; kj < my_R_end; ++kj) {
                    for (ki = 0; ki < cfg.S; ++ki) {
                      ...
                      gemm_param.op.tertiary  = (void*) &n_blocks;
                      gemm_param.a.primary    = (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, pix/cfg.req_dot_modfree_divisor, 0, 0, cfg.blocksofm, cfg.output_pixels/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
                      ...
                      if ((pix + cfg.pixel_blocking >= cfg.n_used_pixels) && (img == my_img_end - img_block_size)) {
                        ...
                        unary_param.out.primary= (void*) dst_ptr;
                        cfg.upd_weight_cvt_f32bf16( &unary_param );
                        unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(7, weight_private_group, ofm1, ifm1, kj, ki, 0, 0, 0, cfg.blocksifm, cfg.R, cfg.S, cfg.ifmblock/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
                        cfg.upd_weight_vnni_format_bf16( &unary_param );
                      }
                    }
                  }
              ...
              for (ofm1 = ofmb; ofm1 < LIBXSMM_MIN(ofmb+cfg.block_upd_ofm, cfg.blocksofm); ofm1++ ) {
                ...
                if (pix == 0 && ifmb == 0) {
                  if (cfg.upd_padding_copy == 1) {
                    ...
                    unary_param.out.primary = (void*) zero_ptr_out;
                    cfg.zero_ofmblock_output_pixels_bf16( &unary_param );
                    if (OFWP % cfg.req_dot_modfree_divisor != 0) {
                      for (oj = 0; oj < cfg.ofhp; oj++) {
                        for (oi = 0; oi < cfg.ofwp; oi++) {
                          for (ofm2 = 0; ofm2 < cfg.ofmblock; ofm2++) {
                            LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, (oj*OFWP+oi)/cfg.req_dot_modfree_divisor, ofm2, (oj*OFWP+oi)%cfg.req_dot_modfree_divisor, cfg.blocksofm, cfg.output_pixels/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor) =
                      ...
                      for (oj = 0; oj < cfg.ofhp; oj++) {
                        unary_param.in.primary = (void*) &LIBXSMM_VLA_ACCESS(5, output, img, ofm1, oj, 0, 0, cfg.blocksofm, cfg.ofhp, cfg.ofwp, cfg.ofmblock);
                        unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, (oj*OFWP)/cfg.req_dot_modfree_divisor, 0, 0, cfg.blocksofm, cfg.output_pixels/cfg.req_dot_modfree_divisor, cfg.ofmblock, cfg.req_dot_modfree_divisor);
                        cfg.vnni_output_w_pixels_bf16( &unary_param );
                      }
                    }
                  } else {
