...
LIBXSMM_API void libxsmm_dnn_conv_upd_exec_bf8( libxsmm_dnn_conv_config cfg, const libxsmm_bfloat8* in_act_ptr, const libxsmm_bfloat8* dout_act_ptr, libxsmm_bfloat8* dfilter_ptr,
  ...
  libxsmm_bfloat8 *const out = (libxsmm_bfloat8*)dout_act_ptr + ((size_t)cfg.pad_h_out * cfg.ofwp + cfg.pad_w_out) * cfg.ofmblock;
  LIBXSMM_VLA_DECL(5, const libxsmm_bfloat8, output, (const libxsmm_bfloat8*)out, cfg.blocksofm, cfg.ofhp, cfg.ofwp, cfg.ofmblock);
  LIBXSMM_VLA_DECL(5, const libxsmm_bfloat8, input, (const libxsmm_bfloat8*)in_act_ptr, cfg.blocksifm, cfg.ifhp, cfg.ifwp, cfg.ifmblock);

  libxsmm_bfloat8 *weight_ptr = (libxsmm_bfloat8*)((char*)scratch + cfg.upd_filter_scratch_offset) + ltid * cfg.C * cfg.K * cfg.R * cfg.S;

  libxsmm_bfloat8 *filter_dst_ptr = (cfg.weight_copies > 1) ? (libxsmm_bfloat8*)weight_ptr : (libxsmm_bfloat8*)dfilter_ptr;
  LIBXSMM_VLA_DECL(7, libxsmm_bfloat8, weight_dst, (libxsmm_bfloat8*)filter_dst_ptr, cfg.blocksifm, cfg.R, cfg.S, cfg.ifmblock/4, cfg.ofmblock, 4);
  ...
  libxsmm_bfloat8 *filter_scratch = (libxsmm_bfloat8*)((char*)scratch + cfg.upd_lp_filter_full_scratch_offset) + ltid * cfg.ofmblock * cfg.ifmblock * 2;

  LIBXSMM_VLA_DECL(2, float, filter_tmp, (float*)filter_scratch, cfg.ofmblock);

  libxsmm_bfloat8 *scratch_tr_input = (libxsmm_bfloat8*)((char*)scratch + cfg.upd_lp_input_full_scratch_offset);
  libxsmm_bfloat8 *zero_ptr_in;
  libxsmm_bfloat8 *zero_ptr_out;
  LIBXSMM_VLA_DECL(4, libxsmm_bfloat8, tr_input, (libxsmm_bfloat8*) scratch_tr_input, cfg.blocksifm, cfg.ifmblock, cfg.input_pixels);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, tr_input_2, (libxsmm_bfloat8*) scratch_tr_input, cfg.blocksifm, cfg.ifmblock, IFHP, cfg.ifwp_extended);

  libxsmm_bfloat8 *scratch_tr_output = (libxsmm_bfloat8*)((char*)scratch + cfg.upd_lp_output_full_scratch_offset);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, tr_output, (libxsmm_bfloat8*) scratch_tr_output, cfg.blocksofm, cfg.output_pixels/4, cfg.ofmblock, 4);
  LIBXSMM_VLA_DECL(6, libxsmm_bfloat8, tr_output_2, (libxsmm_bfloat8*) scratch_tr_output, cfg.blocksofm, OFHP, cfg.ofwp_extended/4, cfg.ofmblock, 4);
  ...
  if (cfg.upd_linearized_pixels == 1) {
        ...
        for (img = my_img_start; img < my_img_end; img++) {
          if (cfg.upd_padding_copy == 1) {
            for (ifm1 = 0; ifm1 < cfg.blocksifm; ifm1++) {
              zero_ptr_in = (libxsmm_bfloat8*) &LIBXSMM_VLA_ACCESS(4, tr_input, img, ifm1, 0, 0, cfg.blocksifm, cfg.ifmblock, cfg.input_pixels);
              unary_param.out.primary = (void*) zero_ptr_in;
              cfg.zero_ifmblock_input_pixels_bf8( &unary_param );
              for (ij = 0; ij < cfg.ifhp; ij++) {
                ...
                cfg.transpose_input_pixels_bf8( &unary_param );
              }
            }
          } else {
            for (ifm1 = 0; ifm1 < cfg.blocksifm; ifm1++) {
        ...
        for (img = my_img_start; img < my_img_end; img++) {
          for (ifm1 = 0; ifm1 < cfg.blocksifm; ifm1++) {
            for (ij = 0; ij < cfg.ifhp/cfg.u; ij++) {
              ...
              cfg.transposeNpack_input_pixels_bf8( &unary_param );
            }
          }
        }
      }
      ...
      if (cfg.upd_padding_copy == 1) {
        for (img = my_img_start; img < my_img_end; img++) {
          for (ofm1 = 0; ofm1 < cfg.blocksofm; ofm1++) {
            zero_ptr_out = (libxsmm_bfloat8*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, 0, 0, 0, cfg.blocksofm, cfg.output_pixels/4, cfg.ofmblock, 4);
            unary_param.out.primary = (void*) zero_ptr_out;
            cfg.zero_ofmblock_output_pixels_bf8( &unary_param );
            if (cfg.ofwp % 4 == 0) {
              for (oj = 0; oj < cfg.ofhp; oj++) {
                ...
                unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, (oj*OFWP)/4, 0, 0, cfg.blocksofm, cfg.output_pixels/4, cfg.ofmblock, 4);
                cfg.vnni_output_w_pixels_bf8( &unary_param );
              }
            } else {
#if 0
              for (oj = 0; oj < cfg.ofhp; oj++) {
                unary_param.in.primary = (void*) &LIBXSMM_VLA_ACCESS(5, output, img, ofm1, oj, 0, 0, cfg.blocksofm, cfg.ofhp, cfg.ofwp, cfg.ofmblock);
                unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, (oj*OFWP)/4, 0, 0, cfg.blocksofm, cfg.output_pixels/4, cfg.ofmblock, 4);
                cfg.vnni_output_w2_pixels_bf8( &unary_param );
                for (ofm2 = 0; ofm2 < cfg.ofmblock; ofm2++) {
                  LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, (oj*OFWP+cfg.ofwp-1)/4, ofm2, (oj*OFWP+cfg.ofwp-1)%4, cfg.blocksofm, cfg.output_pixels/4, cfg.ofmblock, 4) =
              ...
              for (oj = 0; oj < cfg.ofhp; oj++) {
                for (oi = 0; oi < cfg.ofwp; oi++) {
                  for (ofm2 = 0; ofm2 < cfg.ofmblock; ofm2++) {
                    LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, (oj*OFWP+oi)/4, ofm2, (oj*OFWP+oi)%4, cfg.blocksofm, cfg.output_pixels/4, cfg.ofmblock, 4) =
        ...
        for (img = my_img_start; img < my_img_end; img++) {
          for (ofm1 = 0; ofm1 < cfg.blocksofm; ofm1++) {
            ...
            unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, 0, 0, 0, cfg.blocksofm, cfg.output_pixels/4, cfg.ofmblock, 4);
            cfg.vnni_output_compute_pixels_bf8( &unary_param );
            if (cfg.upd_remaining_pixels > 0) {
              unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, (cfg.compute_pixels+1)/4, 0, 0, cfg.blocksofm, cfg.output_pixels/4, cfg.ofmblock, 4);
              cfg.vnni_output_zero_remaining_pixels_bf8( &unary_param );
            }
          }
        }
      }
    ...
    if (cfg.upd_trans_w_only == 0) {
      if (cfg.on_the_fly_input_packing == 0) {
        for (img = my_img_start; img < my_img_end; img++) {
          zero_ptr_in = (libxsmm_bfloat8*) &LIBXSMM_VLA_ACCESS(5, tr_input_2, img, 0, 0, 0, 0, cfg.blocksifm, cfg.ifmblock, IFHP, cfg.ifwp_extended);
          unary_param.out.primary = (void*) zero_ptr_in;
          cfg.zero_ifmblock_input_pixels_extended_bf8( &unary_param );
          for (ifm1 = 0; ifm1 < cfg.blocksifm; ifm1++) {
            for (ij = 0; ij < cfg.ifhp; ij++) {
              unary_param.in.primary = (void*) &LIBXSMM_VLA_ACCESS(5, input, img, ifm1, ij, 0, 0, cfg.blocksifm, cfg.ifhp, cfg.ifwp, cfg.ifmblock);
              ...
              cfg.transpose_input_pixels_ifwp_extended2_bf8( &unary_param );
            }
          }
        }
      }
      for (img = my_img_start; img < my_img_end; img++) {
        for (ofm1 = 0; ofm1 < cfg.blocksofm; ofm1++) {
          for (oj = 0; oj < cfg.ofh; oj++) {
            unary_param.in.primary = (void*) &LIBXSMM_VLA_ACCESS(5, output, img, ofm1, oj, 0, 0, cfg.blocksofm, cfg.ofhp, cfg.ofwp, cfg.ofmblock);
            unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(6, tr_output_2, img, ofm1, oj, 0, 0, 0, cfg.blocksofm, OFHP, cfg.ofwp_extended/4, cfg.ofmblock, 4);
            cfg.vnni_output_w_pixels_bf8( &unary_param );
          }
        }
      }
  ...
  if (cfg.upd_linearized_pixels == 0) {
            ...
            for (ifmb = 0; ifmb < cfg.blocksifm; ifmb += cfg.block_upd_ifm) {
              for (ofm1 = ofmb; ofm1 < LIBXSMM_MIN(ofmb+cfg.block_upd_ofm, cfg.blocksofm); ofm1++ ) {
                ...
                for (j=0; j < cfg.batchreduce_h_pixels; j++) {
                  ...
                  unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(6, tr_output_2, img, 0, j, 0, 0, 0, cfg.blocksofm, OFHP, cfg.ofwp_extended/4, cfg.ofmblock, 4);
                  cfg.vnni_output_w_pixels_bf8( &unary_param );
                }
                for (ifm1 = ifmb; ifm1 < LIBXSMM_MIN(ifmb+cfg.block_upd_ifm, cfg.blocksifm); ifm1++) {
                  ...
                  for (j=0; j < cfg.batchreduce_h_pixels; j++) {
                    ...
                    cfg.transpose_input_pixels_ifwp_extended_bf8( &unary_param );
                  }
                  for (kj = 0; kj < cfg.R; ++kj) {
                    for (ki = 0; ki < cfg.S; ++ki) {
                      ...
                      gemm_param.a.primary    = (void*) &LIBXSMM_VLA_ACCESS(6, tr_output_2, img, 0, 0, 0, 0, 0, cfg.blocksofm, OFHP, cfg.ofwp_extended/4, cfg.ofmblock, 4);
                      ...
                      cfg.upd_compute_kernel1_bf8f32.gemm( &gemm_param );

                      /* Convert fully accumulated buffer to bf8 weight buffer in case of full accumulation has happened */
                      if ((oj + cfg.batchreduce_h_pixels >= cfg.ofh) && (img == my_img_end - 1)) {
                        ...
                        cfg.upd_weight_cvt_f32bf8( &unary_param );
                        unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(7, weight_dst, ofm1, ifm1, kj, ki, 0, 0, 0, cfg.blocksifm, cfg.R, cfg.S, cfg.ifmblock/4, cfg.ofmblock, 4);
                        cfg.upd_weight_vnni_format_bf8( &unary_param );
                      }
                    }
                  }
                }
                ...
                for (ifm1 = ifmb; ifm1 < LIBXSMM_MIN(ifmb+cfg.block_upd_ifm, cfg.blocksifm); ifm1++) {
                  for (kj = 0; kj < cfg.R; ++kj) {
                    for (ki = 0; ki < cfg.S; ++ki) {
                      ...
                      if (cfg.on_the_fly_input_packing == 1) {
                        if ((fast_trans == 1) && (cfg.upd_padding_copy == 0)) {
                          for (ij = 0; ij < cfg.batchreduce_h_pixels; ij++) {
                            ...
                            cfg.transpose_input_pixels_ifwp_strided_extended_bf8( &unary_param );
                          }
                        } else {
                          if (cfg.upd_padding_copy == 1) {
                            for (ij = 0; ij < cfg.batchreduce_h_pixels; ij++) {
                              for (ii = 0; ii < cfg.ofw; ii++) {
                                ...
                                if ( (j_pixel >= cfg.pad_h) && (i_pixel >= cfg.pad_w) && (j_pixel < cfg.ifhp+cfg.pad_h) && (i_pixel < cfg.ifwp+cfg.pad_w) ) {
                                  ...
                                  for (ifm2 = 0; ifm2 < cfg.ifmblock; ifm2++) {
                                    LIBXSMM_VLA_ACCESS(5, tr_input_2, img, 0, ifm2, ij, ii, cfg.blocksifm, cfg.ifmblock, IFHP, cfg.ifwp_extended) = (libxsmm_bfloat8)0;
                                  }
                                }
                              }
                            }
                      ...
                      gemm_param.op.tertiary  = (void*) &n_blocks;
                      gemm_param.a.primary    = (void*) &LIBXSMM_VLA_ACCESS(6, tr_output_2, img, ofm1, oj, 0, 0, 0, cfg.blocksofm, OFHP, cfg.ofwp_extended/4, cfg.ofmblock, 4);
                      if (cfg.on_the_fly_input_packing == 1) {
                        gemm_param.b.primary    = (void*) &LIBXSMM_VLA_ACCESS(5, tr_input_2, img, 0, 0, 0, 0, cfg.blocksifm, cfg.ifmblock, IFHP, cfg.ifwp_extended);
                      } else {
                      ...
                      gemm_param.c.primary    = (void*) dst_ptr;
                      cfg.upd_compute_kernel2_bf8f32.gemm( &gemm_param );
    ...
    if (cfg.use_hybrid_imgofm_parallelization == 1) {
      ...
      int my_R_start = 0;
      int my_R_end = cfg.R;
      libxsmm_bfloat8 *weight_ptr_group = (cfg.weight_copies > 1) ? (libxsmm_bfloat8*)((char*)scratch + cfg.upd_filter_scratch_offset) + tile_id * cfg.C * cfg.K * cfg.R * cfg.S : (libxsmm_bfloat8*)dfilter_ptr;
      LIBXSMM_VLA_DECL(7, libxsmm_bfloat8, weight_private_group, (libxsmm_bfloat8*)weight_ptr_group, cfg.blocksifm, cfg.R, cfg.S, cfg.ifmblock/4, cfg.ofmblock, 4);
      /* This intermediate tensor is used when pixels are NOT fully accumulated  */
                ...
                for (ifm1 = ifmb; ifm1 < LIBXSMM_MIN(ifmb+block_ifm, my_ifm_end); ifm1++) {
                  for (kj = my_R_start; kj < my_R_end; ++kj) {
                    for (ki = 0; ki < cfg.S; ++ki) {
                      ...
                      gemm_param.op.tertiary  = (void*) &n_blocks;
                      gemm_param.a.primary    = (void*) &LIBXSMM_VLA_ACCESS(5, tr_output, img, ofm1, pix/4, 0, 0, cfg.blocksofm, cfg.output_pixels/4, cfg.ofmblock, 4);
                      ...
                      gemm_param.c.primary    = (void*) dst_ptr;
                      cfg.upd_compute_kernel3_bf8f32.gemm( &gemm_param );

                      /* Convert fully caccumulated buffer to bf8 weight buffer in case of full accumulation has happened */
                      if ((pix + cfg.pixel_blocking >= cfg.n_used_pixels) && (img == my_img_end - img_block_size)) {
                        unary_param.in.primary = (void*) dst_ptr;
                        unary_param.out.primary= (void*) dst_ptr;
                        cfg.upd_weight_cvt_f32bf8( &unary_param );
                        unary_param.out.primary= (void*) &LIBXSMM_VLA_ACCESS(7, weight_private_group, ofm1, ifm1, kj, ki, 0, 0, 0, cfg.blocksifm, cfg.R, cfg.S, cfg.ifmblock/4, cfg.ofmblock, 4);
                        cfg.upd_weight_vnni_format_bf8( &unary_param );
                      }
                    }
                  }
                ...
                for (ifm1 = ifmb; ifm1 < LIBXSMM_MIN(ifmb+cfg.block_upd_ifm, cfg.blocksifm); ifm1++) {
                  ...
                  for (kj = 0; kj < cfg.R; ++kj) {
                    for (ki = 0; ki < cfg.S; ++ki) {
                      ...
                      gemm_param.b.primary    = (void*) &LIBXSMM_VLA_ACCESS(4, tr_input, img, ifm1, 0, pix + kj * IFWP + ki, cfg.blocksifm, cfg.ifmblock, cfg.input_pixels);
                      gemm_param.c.primary    = (void*) dst_ptr;
                      cfg.upd_compute_kernel4_bf8f32.gemm( &gemm_param );
  ...
  if (cfg.weight_copies > 1) {
    ...
    libxsmm_barrier_wait(cfg.barrier, ltid);
    unary_param.in.primary  = (void*)((libxsmm_bfloat8*) ((char*)scratch + cfg.upd_filter_scratch_offset) + reduce_thr_begin * fm_blocking);
    unary_param.out.primary = (void*)((libxsmm_bfloat8*) dfilter_ptr + reduce_thr_begin * fm_blocking);
    if ((reduce_thr_end - reduce_thr_begin) == reduce_chunksize) {
      cfg.wt_reduce_kernel0_bf8( &unary_param );
    } else {
      if ((reduce_thr_end - reduce_thr_begin) > 0) {
        cfg.wt_reduce_kernel1_bf8( &unary_param );
      }
    }
    libxsmm_barrier_wait(cfg.barrier, ltid);
  }
