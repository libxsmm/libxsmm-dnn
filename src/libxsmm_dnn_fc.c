...
LIBXSMM_API libxsmm_dnn_fc_fwd_config setup_libxsmm_dnn_fc_fwd(libxsmm_blasint N, libxsmm_blasint C, libxsmm_blasint K, libxsmm_blasint bn,
  ...
  libxsmm_blasint lda = ( layout == LIBXSMM_DNN_FC_LAYOUT_FLAT ) ? K : bk;
  libxsmm_blasint ldb = ( layout == LIBXSMM_DNN_FC_LAYOUT_FLAT ) ? C : bc;
  libxsmm_blasint ldc = ( layout == LIBXSMM_DNN_FC_LAYOUT_FLAT ) ? K : bk;
  ...
  if ( (datatype_in == LIBXSMM_DATATYPE_F32) &&
    ...
    libxsmm_blasint stride_b = ldb*res.bn*sizeof(float);
    ...
    if ( layout == LIBXSMM_DNN_FC_LAYOUT_VNNIPACK_WT ) {
    ...
    } else if ( (layout == LIBXSMM_DNN_FC_LAYOUT_VNNIPACK_WT_IACT_TRANS) || (layout == LIBXSMM_DNN_FC_LAYOUT_VNNIPACK_WT_IACT_TRANS_OACT_TRANS) ) {
    ...
    l_brconfig = libxsmm_create_gemm_batch_reduce_config( LIBXSMM_GEMM_BATCH_REDUCE_STRIDE,
                                                          lda*res.bc*sizeof(libxsmm_bfloat16),
                                                          ldb*res.bn*sizeof(libxsmm_bfloat16),
                                                          unroll_hint );
...
LIBXSMM_API libxsmm_dnn_fc_bwd_config setup_libxsmm_dnn_fc_bwd(libxsmm_blasint N, libxsmm_blasint C, libxsmm_blasint K, libxsmm_blasint bn,
  ...
  if ( l_is_aarch64 == 0 ) {
    if (threads == 16) {
      ...
      res.bwd_row_teams = 8;
      res.bwd_M_hyperpartitions = 1;
      res.bwd_N_hyperpartitions = 8;
      res.upd_bf = 1;
      res.upd_2d_blocking = 1;
      res.upd_col_teams = 1;
      res.upd_row_teams = 8;
      res.upd_M_hyperpartitions = 1;
      res.upd_N_hyperpartitions = 8;
      res.ifm_subtasks = 1;
      res.ofm_subtasks = 1;
  ...
  if ( (datatype_in == LIBXSMM_DATATYPE_F32) &&
    ...
    /* init scratch */
    size_bwd_scratch = sizeof(float) * LIBXSMM_MAX(res.C * res.N, res.threads * res.bc * res.bn) + sizeof(libxsmm_bfloat8) * res.C * res.K;
    res.bwd_private_tr_wt_scratch_mark = size_bwd_scratch;
    ...
    res.upd_private_tr_act_scratch_mark = res.scratch_size;
    res.scratch_size += res.threads * res.bc * res.N * (((res.C/res.bc)+res.upd_col_teams-1)/res.upd_col_teams) * sizeof(libxsmm_bfloat8);
...
LIBXSMM_API void libxsmm_dnn_fc_fwd_exec_f32_flat( libxsmm_dnn_fc_fwd_config cfg, const float* wt_ptr, const float* in_act_ptr, float* out_act_ptr,
  ...
  LIBXSMM_VLA_DECL(4, float,           output, out_act_ptr, cfg.bn, nBlocksOFm, cfg.bk);
  LIBXSMM_VLA_DECL(4, const float,      input,  in_act_ptr, cfg.bn, nBlocksIFm, cfg.bc);
  LIBXSMM_VLA_DECL(4, const float,     filter,      wt_ptr, cfg.bc, nBlocksOFm, cfg.bk);
  LIBXSMM_VLA_DECL(2, const float,       bias,    bias_ptr,                     cfg.bk);
  LIBXSMM_VLA_DECL(4, unsigned char, relubitmask,    relu_ptr, cfg.bn, nBlocksOFm, cfg.bk/8);
          ...
          for (mb1 = my_N_start; mb1 < my_N_end; ++mb1) {
            if ( ifm1 == 0 ) {
              if ( cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS || cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS_RELU || cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS_RELU_WITH_MASK ) {
                  ...
                  gemm_param_ext.a.primary = (void*)&LIBXSMM_VLA_ACCESS(4, filter, ifm1*CB_BLOCKS, 0, ofm1, 0, cfg.bc, nBlocksOFm, cfg.bk);
                  gemm_param_ext.b.primary = (void*)&LIBXSMM_VLA_ACCESS(4, input,  mb1, 0, ifm1*CB_BLOCKS, 0, cfg.bn, nBlocksIFm, cfg.bc);
                  gemm_param_ext.c.primary = (void*)&LIBXSMM_VLA_ACCESS(4, output, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
                ...
                gemm_param.a.primary = (void*)&LIBXSMM_VLA_ACCESS(4, filter, ifm1*CB_BLOCKS, 0, ofm1, 0, cfg.bc, nBlocksOFm, cfg.bk);
                gemm_param.b.primary = (void*)&LIBXSMM_VLA_ACCESS(4, input,  mb1, 0, ifm1*CB_BLOCKS, 0, cfg.bn, nBlocksIFm, cfg.bc);
                gemm_param.c.primary = (void*)&LIBXSMM_VLA_ACCESS(4, output, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
              ...
              if ( ( ifm1 == BF-1 ) &&
                ...
                gemm_param_ext.b.primary   = (void*)&LIBXSMM_VLA_ACCESS(4, input,   mb1, 0, ifm1*CB_BLOCKS, 0, cfg.bn, nBlocksIFm, cfg.bc);
                gemm_param_ext.c.primary   = (void*)&LIBXSMM_VLA_ACCESS(4, output,      mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
                gemm_param_ext.c.secondary = (void*)&LIBXSMM_VLA_ACCESS(4, relubitmask, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk/8);
      ...
      for (ofm1 = my_M_start; ofm1 < my_M_end; ++ofm1) {
        for (mb1 = my_N_start; mb1 < my_N_end; ++mb1) {
          if ( cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS ) { /* bias only */
            ...
            gemm_param_ext.b.primary = (void*)&LIBXSMM_VLA_ACCESS(4, input,  mb1, 0, 0, 0, cfg.bn, nBlocksIFm, cfg.bc);
            gemm_param_ext.c.primary = (void*)&LIBXSMM_VLA_ACCESS(4, output, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
            gemm_param_ext.d.primary = (void*)&LIBXSMM_VLA_ACCESS(2, bias, ofm1, 0, cfg.bk);
            cfg.fwd_compute_kernel2_strd_fused_f32( &gemm_param_ext ); /* beta = 0.0 + bias */
            ...
            gemm_param.op.tertiary = &blocks;
            gemm_param.a.primary = (void*)&LIBXSMM_VLA_ACCESS(4, filter, 0, 0, ofm1, 0, cfg.bc, nBlocksOFm, cfg.bk);
            gemm_param.b.primary = (void*)&LIBXSMM_VLA_ACCESS(4, input,  mb1, 0, 0, 0, cfg.bn, nBlocksIFm, cfg.bc);
            gemm_param.c.primary = (void*)&LIBXSMM_VLA_ACCESS(4, output, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
            cfg.fwd_compute_kernel2_strd( &gemm_param ); /* beta = 0.0 */
          }
        }
...
LIBXSMM_API void libxsmm_dnn_fc_fwd_exec_f32_packed( libxsmm_dnn_fc_fwd_config cfg, const float* wt_ptr, const float* in_act_ptr, float* out_act_ptr,
    const float* bias_ptr, unsigned char* relu_ptr, int start_tid, int my_tid, void* scratch ) {
  const libxsmm_blasint nBlocksIFm = cfg.C / cfg.bc;
  const libxsmm_blasint nBlocksOFm = cfg.K / cfg.bk;
  const libxsmm_blasint nBlocksMB  = cfg.N / cfg.bn;
...
LIBXSMM_API void libxsmm_dnn_fc_fwd_exec_f32( libxsmm_dnn_fc_fwd_config cfg, const float* wt_ptr, const float* in_act_ptr, float* out_act_ptr,
                          const float* bias_ptr, unsigned char* relu_ptr, int start_tid, int my_tid, void* scratch )
{
  if ( cfg.layout == LIBXSMM_DNN_FC_LAYOUT_PACKED )  {
    libxsmm_dnn_fc_fwd_exec_f32_packed( cfg, wt_ptr, in_act_ptr, out_act_ptr, bias_ptr, relu_ptr, start_tid, my_tid, scratch );
  } else {
    libxsmm_dnn_fc_fwd_exec_f32_flat( cfg, wt_ptr, in_act_ptr, out_act_ptr, bias_ptr, relu_ptr, start_tid, my_tid, scratch );
  }
}
                                      ...
                                      const libxsmm_bfloat16* bias_ptr, unsigned char* relu_ptr, int start_tid, int my_tid, void* scratch )
{
  ...
  LIBXSMM_VLA_DECL(4, libxsmm_bfloat16,       output,  out_act_ptr, cfg.bn, nBlocksOFm, cfg.bk);
  LIBXSMM_VLA_DECL(4, const libxsmm_bfloat16,  input,   in_act_ptr, cfg.bn, nBlocksIFm, cfg.bc);
  LIBXSMM_VLA_DECL(4, const libxsmm_bfloat16, filter,       wt_ptr, cfg.bc, nBlocksOFm, cfg.bk);
  LIBXSMM_VLA_DECL(4, float, output_f32, (float*)scratch, bn, nBlocksOFm, bk);
  ...
  LIBXSMM_VLA_DECL(4, unsigned int,  relubitmask, ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) ? (unsigned int*)relu_ptr : NULL, cfg.bn, nBlocksOFm, cfg.bk/32);
          ...
          for (mb1 = my_N_start; mb1 < my_N_end; ++mb1) {
            if ( ifm1 == 0 ) {
              if ( (cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS ) {
                ...
                unary_ld_param.out.primary = &LIBXSMM_VLA_ACCESS(4, output_f32, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
            ...
            gemm_param.a.primary = (void*)&LIBXSMM_VLA_ACCESS(4, filter, ifm1*CB_BLOCKS, 0, ofm1, 0, cfg.bc, nBlocksOFm, cfg.bk);
            gemm_param.b.primary = (void*)&LIBXSMM_VLA_ACCESS(4, input,  mb1, 0, ifm1*CB_BLOCKS, 0,  cfg.bn, nBlocksIFm, cfg.bc);
            gemm_param.c.primary = (void*)&LIBXSMM_VLA_ACCESS(4, output_f32, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
            ...
            if ( ifm1 == BF-1  ) {
              if ( ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU ) ||
                ...
                unary_st_param.in.primary = &LIBXSMM_VLA_ACCESS(4, output_f32, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
                unary_st_param.out.primary = &LIBXSMM_VLA_ACCESS(4, output, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
                if ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) {
                  unary_st_param.out.secondary = &LIBXSMM_VLA_ACCESS(4, relubitmask, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk/32);
                }
      ...
      for (ofm1 = my_M_start; ofm1 < my_M_end; ++ofm1) {
        for (mb1 = my_N_start; mb1 < my_N_end; ++mb1) {
          if ( ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) ||
            ...
            gemm_ext_param.a.primary = (void*)&LIBXSMM_VLA_ACCESS(4, filter, 0, 0, ofm1, 0, cfg.bc, nBlocksOFm, cfg.bk);
            gemm_ext_param.b.primary = (void*)&LIBXSMM_VLA_ACCESS(4, input,  mb1, 0, 0, 0, cfg.bn, nBlocksIFm, cfg.bc);
            gemm_ext_param.c.primary = (void*)&LIBXSMM_VLA_ACCESS(4, output, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
            if ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) {
              gemm_ext_param.d.primary = (void*)&LIBXSMM_VLA_ACCESS(2, bias, ofm1, 0, cfg.bk);
            }
            if ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) {
              gemm_ext_param.c.secondary = (void*)&LIBXSMM_VLA_ACCESS(4, relubitmask, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk/32);
            }
      ...
      for ( ifm1 = 0; ifm1 < BF; ++ifm1 ) {
        for ( mb1ofm1 = thr_begin; mb1ofm1 < thr_end; ++mb1ofm1 ) {
          ...
          if ( ifm1 == 0 ) {
            if ( (cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS ) {
              unary_ld_param.in.primary  = (void*)&LIBXSMM_VLA_ACCESS(2, bias, ofm1, 0,cfg.bk);
              unary_ld_param.out.primary = (void*)&LIBXSMM_VLA_ACCESS(4, output_f32, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
              cfg.fwd_colbcast_load_kernel( &unary_ld_param );
            } else {
              unary_ld_param.out.primary = (void*)&LIBXSMM_VLA_ACCESS(4, output_f32, mb1, 0, ofm1, 0, cfg.bn, nBlocksOFm, cfg.bk);
              cfg.fwd_zero_kernel( &unary_ld_param );
...
LIBXSMM_API void libxsmm_dnn_fc_fwd_exec_bf16_packed( libxsmm_dnn_fc_fwd_config cfg, const libxsmm_bfloat16* wt_ptr, const libxsmm_bfloat16* in_act_ptr, libxsmm_bfloat16* out_act_ptr,
                                      const libxsmm_bfloat16* bias_ptr, unsigned char* relu_ptr, int start_tid, int my_tid, void* scratch )
{
  const libxsmm_blasint nBlocksIFm = cfg.C / cfg.bc;
  const libxsmm_blasint nBlocksOFm = cfg.K / cfg.bk;
                          ...
                          const libxsmm_bfloat16* bias_ptr, unsigned char* relu_ptr, int start_tid, int my_tid, void* scratch )
{
  ...
  } else if ( cfg.layout == LIBXSMM_DNN_FC_LAYOUT_VNNIPACK_WT_IACT_TRANS_OACT_TRANS ) {
  ...
  } else if ( cfg.layout == LIBXSMM_DNN_FC_LAYOUT_PACKED )  {
    libxsmm_dnn_fc_fwd_exec_bf16_packed( cfg, wt_ptr, in_act_ptr, out_act_ptr, bias_ptr, relu_ptr, start_tid, my_tid, scratch );
  } else {
                                      ...
                                      const libxsmm_bfloat8* bias_ptr, unsigned char* relu_ptr, int start_tid, int my_tid, void* scratch )
{
  ...
  libxsmm_blasint mb1ofm1 = 0, mb1 = 0, ofm1 = 0, ifm1 = 0;
  libxsmm_blasint N_tasks_per_thread = 0, M_tasks_per_thread = 0, my_M_start = 0, my_M_end = 0, my_N_start = 0, my_N_end = 0, my_col_id = 0, my_row_id = 0, col_teams = 0, row_teams = 0;
  LIBXSMM_VLA_DECL(4, libxsmm_bfloat8,       output,  out_act_ptr, nBlocksOFm, cfg.bn, cfg.bk);
  LIBXSMM_VLA_DECL(4, const libxsmm_bfloat8,  input,   in_act_ptr,  nBlocksIFm, cfg.bn, cfg.bc);
  LIBXSMM_VLA_DECL(5, const libxsmm_bfloat8, filter,       wt_ptr, nBlocksIFm, bc_lp, cfg.bk, lpb);
  LIBXSMM_VLA_DECL(4, float, output_f32, (float*)scratch, nBlocksOFm, bn, bk);
  LIBXSMM_VLA_DECL(2, const libxsmm_bfloat8, bias, ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) ? (libxsmm_bfloat8*) bias_ptr : NULL, cfg.bk);
  LIBXSMM_VLA_DECL(4, unsigned int,  relubitmask, ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU) ? (unsigned int*)relu_ptr : NULL, nBlocksOFm, cfg.bn, cfg.bk/64);
          ...
          for (mb1 = my_N_start; mb1 < my_N_end; ++mb1) {
            ...
            if ( ifm1 == BF-1  ) {
              if ( ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU ) ||
                ...
                unary_st_param.out.primary = &LIBXSMM_VLA_ACCESS(4, output, mb1, ofm1, 0, 0, nBlocksOFm, cfg.bn, cfg.bk);
                if ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) {
                  unary_st_param.out.secondary = &LIBXSMM_VLA_ACCESS(4, relubitmask, mb1, ofm1, 0, 0, nBlocksOFm, cfg.bn, cfg.bk/64);
                }
                cfg.fwd_act_kernel( &unary_st_param );
              } else {
                unary_st_param.in.primary = &LIBXSMM_VLA_ACCESS(4, output_f32, mb1, ofm1, 0, 0, nBlocksOFm, cfg.bn, cfg.bk);
      ...
      for (ofm1 = my_M_start; ofm1 < my_M_end; ++ofm1) {
        for (mb1 = my_N_start; mb1 < my_N_end; ++mb1) {
          if ( ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) ||
            ...
            if ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK) {
              gemm_ext_param.c.secondary = (void*)&LIBXSMM_VLA_ACCESS(4, relubitmask, mb1, ofm1, 0, 0, nBlocksOFm, cfg.bn, cfg.bk/64);
            }
            cfg.fwd_compute_kernel5_strd_fused( &gemm_ext_param );
          } else {
            gemm_param.op.tertiary = (void*)&blocks;
...
LIBXSMM_API void libxsmm_dnn_fc_fwd_exec_bf8( libxsmm_dnn_fc_fwd_config cfg, const libxsmm_bfloat8* wt_ptr, const libxsmm_bfloat8* in_act_ptr, libxsmm_bfloat8* out_act_ptr,
                          const libxsmm_bfloat8* bias_ptr, unsigned char* relu_ptr, int start_tid, int my_tid, void* scratch )
{
  libxsmm_dnn_fc_fwd_exec_bf8_vnni_format( cfg, wt_ptr, in_act_ptr, out_act_ptr, bias_ptr, relu_ptr, start_tid, my_tid, scratch );
}
                                      ...
                                      libxsmm_bfloat16* dbias_ptr, const unsigned char* relu_ptr, libxsmm_dnn_fc_pass pass, int start_tid, int my_tid, void* scratch )
{
  ...
  libxsmm_bfloat16 *grad_output_ptr = ((cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_RELU || cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS_RELU ||
                              cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK || cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS_RELU_WITH_MASK)) ? (libxsmm_bfloat16*)((char*)scratch + cfg.doutput_scratch_mark) : (libxsmm_bfloat16*)dout_act_ptr;
  libxsmm_bfloat16 *tr_doutput_ptr = ((cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_RELU || cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS_RELU ||
                              cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_RELU_WITH_MASK || cfg.fuse_type == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS_RELU_WITH_MASK)) ? (libxsmm_bfloat16*)grad_output_ptr + cfg.N * cfg.K : (libxsmm_bfloat16*)scratch;
...
LIBXSMM_API void libxsmm_dnn_fc_bwd_exec_bf8_vnni_format( libxsmm_dnn_fc_bwd_config cfg,  const libxsmm_bfloat8* wt_ptr, libxsmm_bfloat8* din_act_ptr,
                                      const libxsmm_bfloat8* dout_act_ptr, libxsmm_bfloat8* dwt_ptr, const libxsmm_bfloat8* in_act_ptr,
                                      libxsmm_bfloat8* dbias_ptr, const unsigned char* relu_ptr, libxsmm_dnn_fc_pass pass, int start_tid, int my_tid, void* scratch )
{
  /* size variables, all const */
  /* here we assume that input and output blocking is similar */
  const libxsmm_blasint bn = cfg.bn;
  ...
  LIBXSMM_VLA_DECL(2, libxsmm_bfloat8, dbias, ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) == LIBXSMM_DNN_FC_ELTW_FUSE_BIAS) ? (libxsmm_bfloat8*) dbias_ptr : NULL, cfg.bk);
  LIBXSMM_VLA_DECL(4, unsigned int, relubitmask, ((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU) ? (unsigned int*)relu_ptr : NULL, nBlocksOFm, cfg.bn, cfg.bk/64);

  libxsmm_bfloat8 *grad_output_ptr = (((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU)) ? (libxsmm_bfloat8*)((char*)scratch + cfg.doutput_scratch_mark) : (libxsmm_bfloat8*)dout_act_ptr;
  libxsmm_bfloat8 *tr_doutput_ptr = (((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU)) ? (libxsmm_bfloat8*)grad_output_ptr + cfg.N * cfg.K : (libxsmm_bfloat8*)scratch;
  LIBXSMM_VLA_DECL(4, const libxsmm_bfloat8,   doutput_orig, dout_act_ptr, nBlocksOFm, bn, bk);
  LIBXSMM_VLA_DECL(4, libxsmm_bfloat8,   doutput, grad_output_ptr, nBlocksOFm, bn, bk);
  LIBXSMM_VLA_DECL(5, libxsmm_bfloat8, doutput_tr, tr_doutput_ptr, nBlocksMB, bn_lp, bk, lpb);
  ...
  if (((cfg.fuse_type & LIBXSMM_DNN_FC_ELTW_FUSE_RELU) == LIBXSMM_DNN_FC_ELTW_FUSE_RELU)) {
    for ( mb1ofm1 = eltwise_thr_begin; mb1ofm1 < eltwise_thr_end; ++mb1ofm1 ) {
      ...
      act_param.in.primary   = (void*) &LIBXSMM_VLA_ACCESS(4, doutput_orig, mb1, ofm1, 0, 0, nBlocksOFm, cfg.bn, cfg.bk);
      act_param.out.primary  = (void*)&LIBXSMM_VLA_ACCESS(4, doutput, mb1, ofm1, 0, 0, nBlocksOFm, cfg.bn, cfg.bk);
      act_param.in.secondary = (void*)&LIBXSMM_VLA_ACCESS(4, relubitmask, mb1, ofm1, 0, 0, nBlocksOFm, cfg.bn, cfg.bk/64);
      cfg.bwd_relu_kernel( &act_param );
  ...
  if ( (pass & LIBXSMM_DNN_FC_PASS_BWD_D) == LIBXSMM_DNN_FC_PASS_BWD_D ){
    ...
    LIBXSMM_VLA_DECL(5, const libxsmm_bfloat8, filter,    wt_ptr, nBlocksIFm, bc_lp, bk, lpb);
    LIBXSMM_VLA_DECL(4,       libxsmm_bfloat8, dinput,    din_act_ptr, nBlocksIFm, bn, bc);
    LIBXSMM_VLA_DECL(5,       libxsmm_bfloat8, filter_tr, (libxsmm_bfloat8*)scratch, nBlocksOFm, bk_lp, bc, lpb);
    float* temp_output = (float*)scratch + (cfg.C * cfg.K)/2;
    LIBXSMM_VLA_DECL(4,        float,           dinput_f32, (float*) temp_output, nBlocksIFm, bn, bc);
    ...
    if (use_2d_blocking == 1) {
      LIBXSMM_VLA_DECL(4, libxsmm_bfloat8, tmp_filter_tr, ((libxsmm_bfloat8*)((char*)scratch + cfg.bwd_private_tr_wt_scratch_mark)) + ltid * bc * cfg.K, bk_lp, bc, lpb);
      if (BF > 1) {
        for ( ofm1 = 0; ofm1 < BF; ++ofm1 ) {
          for (ifm1 = my_M_start; ifm1 < my_M_end; ++ifm1) {
            for (ofm2 = ofm1*KB_BLOCKS; ofm2 < (ofm1+1)*KB_BLOCKS; ofm2++) {
  ...
  if ( (pass & LIBXSMM_DNN_FC_PASS_BWD_W) == LIBXSMM_DNN_FC_PASS_BWD_W ) {
    ...
    LIBXSMM_VLA_DECL(4, const libxsmm_bfloat8,  input,    (libxsmm_bfloat8* )in_act_ptr, nBlocksIFm, bn, bc);
    LIBXSMM_VLA_DECL(5,       libxsmm_bfloat8, dfilter,  (libxsmm_bfloat8*)dwt_ptr, nBlocksIFm, bc_lp, bk, lpb);

    /* Set up tensors for transposing/scratch before vnni reformatting dfilter */
    libxsmm_bfloat8  *tr_inp_ptr = (libxsmm_bfloat8*) ((libxsmm_bfloat8*)scratch + cfg.N * cfg.K);
    float               *dfilter_f32_ptr = (float*) ((libxsmm_bfloat8*)tr_inp_ptr + cfg.N * cfg.C);

    LIBXSMM_VLA_DECL(4, libxsmm_bfloat8,  input_tr,    (libxsmm_bfloat8*)tr_inp_ptr, nBlocksMB, bc, bn);
    LIBXSMM_VLA_DECL(4,       float, dfilter_f32,  (float*)dfilter_f32_ptr, nBlocksIFm, bc, bk);
    ...
    if (use_2d_blocking == 1) {
      LIBXSMM_VLA_DECL(4, libxsmm_bfloat8,  tmp_input_tr, ((libxsmm_bfloat8*)((char*)scratch + cfg.upd_private_tr_act_scratch_mark)) + ltid * bc * cfg.N * N_tasks_per_thread, nBlocksMB, bc, bn);
      LIBXSMM_VLA_DECL(4, libxsmm_bfloat8, tmp_doutput_tr, ((libxsmm_bfloat8*)((char*)scratch + cfg.upd_private_tr_dact_scratch_mark)) + ltid * bk * cfg.N, bn_lp, bk, lpb);
      ifm2 = 0;
      ofm2 = 0;
      if (BF == 1) {
        for (ofm1 = my_M_start; ofm1 < my_M_end; ++ofm1) {
        ...
        for (bfn = 0; bfn < BF; bfn++) {
          for (ofm1 = my_M_start; ofm1 < my_M_end; ++ofm1) {
            ...
            for (ifm1 = my_N_start; ifm1 < my_N_end; ++ifm1) {
              ...
              /* Downconvert result to BF16 and vnni format */
              if (bfn == BF-1) {
                LIBXSMM_ALIGNED(libxsmm_bfloat8 tmp_buf[bc][bk], 64);
                copy_param.in.primary    = (void*)&LIBXSMM_VLA_ACCESS(4, dfilter_f32, ofm1, ifm1, 0, 0, nBlocksIFm, bc, bk);
                copy_param.out.primary   = (void*)tmp_buf;
                format_param.in.primary  = (void*)tmp_buf;
                format_param.out.primary = (void*)&LIBXSMM_VLA_ACCESS(5, dfilter, ofm1, ifm1, 0, 0, 0, nBlocksIFm, bc_lp, bk, lpb);
...
LIBXSMM_API void libxsmm_dnn_fc_bwd_exec_bf8( libxsmm_dnn_fc_bwd_config cfg,  const libxsmm_bfloat8* wt_ptr, libxsmm_bfloat8* din_act_ptr,
                          const libxsmm_bfloat8* dout_act_ptr, libxsmm_bfloat8* dwt_ptr, const libxsmm_bfloat8* in_act_ptr,
                          libxsmm_bfloat8* dbias_ptr, const unsigned char* relu_ptr, libxsmm_dnn_fc_pass pass, int start_tid, int my_tid, void* scratch )
{
  libxsmm_dnn_fc_bwd_exec_bf8_vnni_format( cfg, wt_ptr, din_act_ptr, dout_act_ptr, dwt_ptr, in_act_ptr,
                                   dbias_ptr, relu_ptr, pass, start_tid, my_tid, scratch );
}
